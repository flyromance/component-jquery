...
(function() {
    ...
    Page.prototype.view = function() {
        ...
        conf.first = 'first' in conf ? conf.first : '&#x9996;&#x9875;'; // 首页
        conf.last = 'last' in conf ? conf.last : '&#x5C3E;&#x9875;'; // 尾页
        conf.prev = 'prev' in conf ? conf.prev : '&#x4E0A;&#x4E00;&#x9875;'; // 上一页
        conf.next = 'next' in conf ? conf.next : '&#x4E0B;&#x4E00;&#x9875;'; // 下一页
        ...
        if (conf.pages > conf.groups && dict.end < conf.pages && conf.last && conf.groups !== 0) {
            view.push('<span>&#x2026;</span><a href="javascript:;" class="laypage_last" title="&#x5C3E;&#x9875;"  data-page="' +
                conf.pages + '">' + conf.last + '</a>');
        }
        ...
        if (conf.skip) {
            (function() {
                var skip_str = '<span class="laypage_total"><label>&#x5230;&#x7B2C;</label><input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>' +
                    '<button type="button" class="laypage_btn">&#x786e;&#x5b9a;</button></span>';
                view.push(skip_str);
            })();
        }

        layout.push('<div name="laypage' + laypage.v + '"');

        // 自定义皮肤
        conf.skin = conf.skin ? /^#/.test(conf.skin) ? 'molv' : conf.skin : 'default';
        layout.push(' class="laypage_main laypageskin_' + conf.skin + '" id="laypage_' + that.config.item + '">');

        // 中间内容
        layout.push(view.join('') + '</div>');
